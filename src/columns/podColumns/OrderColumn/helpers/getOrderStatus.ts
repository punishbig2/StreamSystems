...
export const getOrderStatus = (
  ...
  const ownOrder: Order | undefined = depth.find((order: Order) => order.user === user.email);
  ...
  status |=
    !!ownOrder && ownOrder.orderId !== target.orderId ? OrderStatus.HasMyOrder : OrderStatus.None;
  status |=
  ...
  if ((status & OrderStatus.SameBank) !== 0 && isBroker) status |= OrderStatus.OwnedByBroker;
  // If the size of the order doesn't match the aggregated size, it's a joined order
  status |= target.size !== aggregatedSize ? OrderStatus.Joined : OrderStatus.None;
  ...
  status &= isBroker && target.firm !== personality ? ~OrderStatus.Owned : ~OrderStatus.None;
