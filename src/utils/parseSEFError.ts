...
const extractKey = (value: string): string => {
  ...
  const sections: string[] = matches[1].split('/').map((key: string): string => {
...
export const parseSEFError = (value: string): readonly SEFErrorEntry[] => {
  const lines: string[] = value.split('\n');
  const parsed: { [index: number]: SEFErrorEntry } = lines.reduce(
    (value: { [index: number]: SEFErrorEntry }, line: string, index: number): any => {
