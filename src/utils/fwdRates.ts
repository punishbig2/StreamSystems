...
const generatePoints = (when: Date, value: number, distance: number): InternalPoint[] => {
...
const comparePoints = ({ date: d1 }: InternalPoint, { date: d2 }: InternalPoint): number => {
...
export const buildFwdRates = (
  ...
  if (fwdrate1 !== null && fwdrate2 !== null) {
    const points: InternalPoint[] = generatePoints(tenor1.deliveryDate, fwdrate1, 1);
    ...
    const points: InternalPoint[] = generatePoints(tenor1.deliveryDate, value, 1);
    if (strategy.spreadvsvol === 'spread' || strategy.spreadvsvol === 'both') {
      if (isTenor(tenor2)) {
