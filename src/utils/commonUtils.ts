...
export const findDefaultTimezone = (): string => {
...
export const percent = (numerator: number, denominator: number, base: number): string => {
...
export const selectInputText = (input: HTMLInputElement): void => input && input.select();
...
export const tryToNumber = (
  ...
  if (asNumber !== null && asNumber !== undefined && !isNaN(asNumber)) return asNumber.toString();
