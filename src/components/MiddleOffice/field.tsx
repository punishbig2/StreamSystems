...
export const FormField: React.FC<Props> = (props: Props) => {
  const value: string = getValue(props.type, props.name, props.value, props.precision);
  const classes: string[] = [props.color];
  if (typeof props.value === 'number' && props.value < 0)
    classes.push('negative');
  const control: ReactElement = (
    <OutlinedInput value={value} name={props.name} readOnly={props.readOnly} labelWidth={0}/>
  );
  return (
