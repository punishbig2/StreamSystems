...
export const Header: <T extends unknown>(props: HeaderProps) => any = <T extends unknown>(props: HeaderProps) => {
  ...
  const columnMapper = (weight: number) => (column: ColumnSpec) => {
    const handleSorting = (): [SortDirection, () => void] => {
      const sortInfo: SortInfo | undefined = sortColumns.find((info: SortInfo) => info.column === column.name);
      const direction = sortInfo ? sortInfo.direction : SortDirection.None;
      const onSorted = () => {
        switch (direction) {
          case SortDirection.Ascending:
            props.addSortColumn({column: column.name, direction: SortDirection.Descending});
            break;
          case SortDirection.Descending:
            props.addSortColumn({column: column.name, direction: SortDirection.None});
            break;
          case SortDirection.None:
            props.addSortColumn({column: column.name, direction: SortDirection.Ascending});
            break;
        }
      };
