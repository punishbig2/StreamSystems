...
export const TradingWorkspace: React.FC<Props> = observer((props: Props): ReactElement | null => {
  ...
  React.useEffect((): void | VoidFunction => {
    ...
    const timer = setInterval((): void => {
      const someTileHasOrders = tiles.some((tile: TileStore): boolean => tile.hasOrders);
  ...
  return (
    <div className={props.visible ? 'visible' : 'hidden'}>
      ...
      <ModalWindow
        render={() => <UserProfileModal onCancel={store.hideUserProfileModal} />}
