...
const reducer: Reducer<State, Action<ActionTypes>> = (state: State, {data, type}: Action<ActionTypes>): State => {
  switch (type) {
...
export const UserProfileForm: React.FC<Props> = (props: Props) => {
  ...
  const [state, dispatch] = useReducer<Reducer<State, Action<ActionTypes>>>(reducer, {
  ...
  const onChange = ({target}: React.ChangeEvent<SelectEventData>) => {
    dispatch(createAction<ActionTypes>(ActionTypes.SetFieldValue, {
      name: target.name,
      value: target.value,
    }));
  };
  ...
  return (
    <>
      ...
      <form className={'user-profile-form'} name={'user-profile'} onSubmit={onSubmit} autoComplete={'off'} noValidate>
        <Grid container direction={'column'}>
            ...
            <Grid item xs={4}>
              <FormControl margin={'normal'} fullWidth>
                ...
                <Select id={'user-type'} onChange={onChange} name={'userType'} value={state.userType}>
                  ...
                  <MenuItem value={UserTypes.MarketMaker}>Market Maker</MenuItem>
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={4}>
          ...
          <Grid item container spacing={2} direction={'row'}>
            <Grid item xs={6}>
              <FormControl margin={'normal'} fullWidth>
                ...
                <Select id={'font'} onChange={onChange} name={'font'} value={state.font}>
            ...
            <Grid item xs={2}>
              <FormControl margin={'normal'} fullWidth>
                ...
                <Select id={'font-size'} onChange={onChange} name={'fontSize'} value={state.fontSize}>
            ...
            <Grid item xs={4}>
              <FormControl margin={'normal'} fullWidth>
                ...
                <Select id={'exec-sound'} onChange={onChange} name={'execSound'} value={state.execSound}>
                  <MenuItem value={'default'}>Default</MenuItem>
                </Select>
              </FormControl>
            </Grid>
          ...
          <Grid item>
            <FormControl margin={'normal'} fullWidth>
              ...
              <Select id={'ccy-group'} onChange={onChange} name={'ccyGroup'} value={state.ccyGroup} displayEmpty={true}
                      renderValue={renderCCYGroup}>
                <MenuItem value={CurrencyGroups.G10}>{CurrencyGroups.G10}</MenuItem>
                <MenuItem value={CurrencyGroups.Asia}>{CurrencyGroups.Asia}</MenuItem>
              </Select>
            </FormControl>
          </Grid>
          ...
          <Grid item>
            <FormControl margin={'normal'} fullWidth>
              ...
              <Select id={'time-zone'} onChange={onChange} name={'timezone'} value={state.timezone} displayEmpty
                      renderValue={renderTimezone}>
                {timezones.map((zone: TimezoneInfo) => (
                  <MenuItem key={zone.text} value={zone.text}>{zone.text}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </Grid>
          ...
          <Grid item>
            <FormControl fullWidth margin={'normal'}>
              ...
              <Select id={'color-scheme'} onChange={onChange} name={'colorScheme'} value={state.colorScheme}>
                <MenuItem value={'default'}>Default</MenuItem>
              </Select>
            </FormControl>
          </Grid>
        ...
        <div className={'modal-buttons'}>
          <button className={'cancel'} onClick={props.onCancel} type={'button'}>{strings.Cancel}</button>
          <button className={'success'} type={'submit'}>{strings.Save}</button>
        </div>
      </form>
    </>
  );
