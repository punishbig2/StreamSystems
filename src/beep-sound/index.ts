...
export const getSoundsList = async (): Promise<ExecSound[]> => {
  return new Promise(
    (resolve: (sounds: ExecSound[]) => void, reject: () => void) => {
      openDatabase((db: IDBDatabase) => {
        const tx: IDBTransaction = db.transaction("list", "readwrite");
        const list: IDBObjectStore = tx.objectStore("list");
        ...
        keysRequest.onsuccess = () => {
          ...
          Promise.all(
            keys.map(
              (key: IDBValidKey): Promise<ExecSound> => {
                return new Promise(
                  (resolve: (sound: ExecSound) => void, reject) => {
                    const finalRequest: IDBRequest<any | undefined> = list.get(
                      key
                    );
...
export const getSound = async (name: string): Promise<ExecSound> => {
  return new Promise(
    (resolve: (sound: ExecSound) => void, reject: () => void) => {
      openDatabase((db: IDBDatabase) => {
        const tx: IDBTransaction = db.transaction("list", "readwrite");
        const list: IDBObjectStore = tx.objectStore("list");
